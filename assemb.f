      subroutine assemb(  NGSK,   neq,  neqm, NGSKo,  neqo,
...
c ****** Initialization ************************************************
      sk = 0.d0 ! sk(:) = 0.d0
c
c ****** Loop for ALL Elements ***********88****************************
      do 100 nel=1,nelx
c
c ===== Initialization of Elemental Stiffness =====
        ske = 0.d0 ! ske(:,:) = 0.d0
c
...
c     --- /QUAD4/ 4-node Isoparametric Element
        if( melem(nel).eq.24 ) then
          ...
          enddo
c
          CALL quad4a(   nel,  nelx,   ndf,  node, ngaus,
     &                 prope,   ske,  ijke,    xe,
     &                dndx_g, det_g,ctensg,
     &                ierror )
...
c     --- /TRIA3/ 3-node Constant Strain Triangle
        elseif( melem(nel).eq.23 ) then
          ...
          enddo
c
          CALL tria3a(   nel,  nelx,   ndf,  node, ngaus,
     &                 prope,   ske,  ijke,    xe,
     &                dndx_g, det_g,ctensg,
     &                ierror )
          if(ierror.ne.0) RETURN
...
c     --- /QUAD8/ 8-node Isoparametric Element (2nd order)
        elseif( melem(nel).eq.28 ) then
          ...
          enddo
c
          CALL quad8a(   nel,  nelx,   ndf,  node, ngaus,
     &                 prope,   ske,  ijke,    xe,
     &                dndx_g, det_g,ctensg,
     &                ierror )
          if(ierror.ne.0) RETURN
...
c        === Usual 6-node parabolic element ===
            CALL tria6a(   nel,  nelx,   ndf,  node, ngaus,
     &                   prope,   ske,  ijke,    xe,
     &                  dndx_g, det_g,ctensg,
     &                  ierror )
c
...
c        === P1-iso-P2/P0 element ===
            CALL triap1(   nel,  nelx,   ndf,  node, ngaus,
     &                   prope,   ske,  ijke,    xe,
     &                  dndx_g, det_g,ctensg,
     &                  ierror )
          endif
...
c     --- /HEXA8/ 8-node Isoparametric Element
        elseif( melem(nel).eq.38 ) then
          ...
          enddo
c
          CALL hexa8a(   nel,  nelx,   ndf,  node, ngaus,
     &                 prope,   ske,  ijke,    xe,
     &                dndx_g, det_g,ctensg,
     &                ierror )
          if(ierror.ne.0) RETURN
...
c
c ===== Assemble Global Stiffness Matrix "sk"
c                  in Compressed Row-major Storage (CRS) format =====
        if( (isolvr.eq.1).or.(isolvr.eq.2) ) then
          CALL mapcrs(   nel,  nelx,   ndf,  node,
     ...
     &                ierror )
          if(ierror.ne.0) RETURN
c
c ===== Assemble Global Stiffness Matrix "SK" from "ske"
c                  in SKYLINE storage format =====
        elseif( isolvr.eq.0) then
          CALL mapsky(   nel,  nelx,   ndf,  node,
     &                  NGSK,   neq,  neqm, NGSKo,  neqo,
     &                   ijk,  mdof, jdiag,index0,index1,
     &                    sk,   ske,
     &                ierror )
...
c **********************************************************************
c     write(*,*) sk
c     write(*,'(/,i5,e12.3)') 1,sk(1)
c     do ne=2,nx*ndf
c       write(*,'(i5,100e10.2)') ne,(sk(nn),nn=jdiag(ne-1)+1,jdiag(ne))
c     enddo
c
c **********************************************************************
c **********************************************************************
